...
```{r check_immconsec}
# DIRECT CONNECTIONS
# If a SAB is directly connected to the preceding SAB, both should be summarised
...
#SAB: remove cases with keep == FALSE
SAB <- SAB %>%
...
SAB <- SAB %>% 
  group_by(pragmaid) %>%
  arrange(date.treat.start) %>%
  mutate(SAB_id = row_number() - 1, # Start at 0 to align with the REHA_count_before_SAB and QEB_count_before_SAB variables, 
    n_SAB_abgebrochen = cumsum(lag(unpl_drop == "ja", default = FALSE)),
    n_SAB_regulaer    = cumsum(lag(unpl_drop != "ja", default = FALSE))) %>%
  ungroup()
...
diagnoses_SAB_employ_fil <- diagnoses_SAB_employ_alltimes %>%
  filter(
    # outpatient diagnoses (duration: 1 quarter) must not have been made more than 2 quarters before the start of SAB and must not be later than the start of SAB
...
nrow(distinct(diagnoses_SAB_employ, pragmaid)) - nrow(distinct(fost_SAB, pragmaid)) # how many patients do have fosterage data?
...
nrow(distinct(fost_diagnoses_SAB_employ_fil, pragmaid)) # how many patients do have fosterage data within the SAB period?
...
med_fost_diagnoses_SAB_employ_fil <- med_fost_diagnoses_SAB_employ_alltimes %>%
  # only keep medication data that happened during the month before the SAB
...
#calculate sum of distinct level 2 ATC codes per SAB-person combi
med_fost_diagnoses_SAB_employ_fil <- med_fost_diagnoses_SAB_employ_fil %>%
  group_by(id_pragma_SAB) %>%
...
med_fost_diagnoses_SAB_employ_QEB <- bind_rows(
  med_fost_diagnoses_SAB_employ, QEB %>% select(pragmaid, date.treat.start, treatment, date.treat.end, unpl_drop))
...
table(med_fost_diagnoses_SAB_employ_QEB$QEB_count_before_SAB, useNA = "always") # 0 NAs
table(med_fost_diagnoses_SAB_employ_QEB$n_QEB_abgebrochen, useNA = "always") # 0 NAs
table(med_fost_diagnoses_SAB_employ_QEB$n_QEB_regulaer, useNA = "always") # 0 NAs
...
med_fost_diagnoses_SAB_employ_QEB_reha <- bind_rows(
  med_fost_diagnoses_SAB_employ_QEB, reha %>% select(pragmaid, date.treat.start, treatment, date.treat.end, unpl_drop))
...
med_fost_diagnoses_SAB_employ_QEB_reha <- med_fost_diagnoses_SAB_employ_QEB_reha %>%
  ...
  mutate(REHA_count_before_SAB = sapply(1:n(), function(i) {
    if (treatment[i] == "SAB") {
      sum(treatment[1:(i-1)] == "drvreha" & date.treat.end[1:(i-1)] < date.treat.start[i])
    } else {
      NA
    }
  }),
    ...
    n_REHA_abgebrochen = sapply(1:n(), function(i) {
      if (treatment[i] == "SAB") {
        sum(treatment[1:(i-1)] == "drvreha" & unpl_drop[1:(i-1)] == "ja" & date.treat.end[1:(i-1)] < date.treat.start[i])
      } else {
        NA
      }
    }),
...
table(med_fost_diagnoses_SAB_employ_QEB_reha$REHA_count_before_SAB, useNA = "always") # 0 NAs
table(med_fost_diagnoses_SAB_employ_QEB_reha$n_REHA_abgebrochen, useNA = "always") # 0 NAs
table(med_fost_diagnoses_SAB_employ_QEB_reha$n_REHA_regulaer, useNA = "always") # 0 NAs
```
...
final_data <- med_fost_diagnoses_SAB_employ_QEB_reha %>% 
  select(-c(KH_FALL_ID, date.treat.start, date.treat.end, date.emp.start, date.emp.end, id_pragma_SAB, fost.degree, source, died, date.death, yob, treatment, QEB_count_before_SAB, REHA_count_before_SAB, SAB_id))
...
Update: elix_sum, n_QEB_abgebrochen, n_SAB_abgebrochen

#### Kategorisierung von elix_sum, n_QEB_abgebrochen und n_SAB_abgebrochen
```{r categorize}
# elixhauser
limits_elix_sum <- quantile(final_data$elix_sum[final_data$elix_sum > 0], probs = c(0.5))
final_data <- final_data %>% mutate(
  elix_sum_cat = case_when(
    elix_sum == 0 ~ "0",
    elix_sum <= limits_elix_sum[1] ~ "1-3",
    TRUE ~ "4+"
  ) %>% factor(levels = c("0", "1-3", "4+"))
)

table(final_data$elix_sum, final_data$elix_sum_cat, useNA = "always")
...
```{r multicollinearity}
vif_model <- glm(unpl_drop ~ sex + age_cent + nationality + emp.type + elix_sum_cat + fost.degree_bin + ndistinctATClevel2_cent + n_QEB_abgebrochen_cat + n_REHA_abgebrochen_cent + n_SAB_abgebrochen_cat + n_QEB_regulaer_cent + n_REHA_regulaer_cent + n_SAB_regulaer_cent, family = "binomial", data = final_data)
...
model8 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + (1|pragmaid), family = "binomial", data = final_data)
summary(model8)
add_model_summary(model8, "Model 8") 

model9 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model9)
...
model11 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model11)
add_model_summary(model11, "Model 11") 

model12 <- glmmTMB(unpl_drop ~ sex + age_cent + nationality + emp.type + elix_sum_cat + fost.degree_bin + ndistinctATClevel2_cent + n_QEB_abgebrochen_cat + n_REHA_abgebrochen_cent + n_SAB_abgebrochen_cat + n_QEB_regulaer_cent + n_REHA_regulaer_cent + n_SAB_regulaer_cent + (1|pragmaid), family = "binomial", data = final_data) 
summary(model12)
...
predictors <- c("sex", "age_cent", "nationality", "emp.type", "elix_sum_cat", 
...
# 
# 
# model13 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*age_cent + (1|pragmaid), family = "binomial", data = final_data)
# summary(model13)
# add_model_summary(model13, "Model 13")
# 
# model14 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*sex + (1|pragmaid), family = "binomial", data = final_data) 
# summary(model14)
# add_model_summary(model14, "Model 14") 
# 
# model15 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*elix_sum_cent + (1|pragmaid), family = "binomial", data = final_data)
# summary(model15)
# add_model_summary(model15, "Model 15") 
# 
# model16 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*nationality + (1|pragmaid), family = "binomial", data = final_data) 
# summary(model16) 
# add_model_summary(model16, "Model 16") 
# 
# model17 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*emp.type + (1|pragmaid), family = "binomial", data = final_data)
# summary(model17) 
# add_model_summary(model17, "Model 17")
# 
# model18 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*fost.degree_bin + (1|pragmaid), family = "binomial", data = final_data)
# summary(model18)
# add_model_summary(model18, "Model 18")
# 
# model19 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*ndistinctATClevel2_cent + (1|pragmaid), family = "binomial", data = final_data)
# summary(model19)
# add_model_summary(model19, "Model 19")
# 
# model20 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*QEB_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
# summary(model20) 
# add_model_summary(model20, "Model 20")
# 
# model21 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QEB_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*REHA_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
# summary(model21) 
...
saveRDS(best_model, file = file.path(folder_models, "full_model_SAB.rds"))
```

## Model Diagnostik: ICC and R2
```{r model_iccr2}
#load the best model (no need to run the model again)
best_model <- readRDS(file.path(folder_models, "full_model_SAB.rds"))
...
# number of distinct pragmaIDs with SAB_id > 0 / number of all distinct pragmaIDs = proportion of patients with more than one SAB
...
new_labels <- c(
  ...
  "elix_sum_cat1-3" = "Elixhauser-Score: 1-3 (Ref.: 0)",
  "elix_sum_cat4+" = "Elixhauser-Score: 4+ (Ref.: 0)",
...
#save as tif
ggsave(file.path(folder_plot, "coefffix_plot_SAB.tif"), coefffix_plot, 
       width = 175,
       height = 142, 
       units = "mm",
       dpi = 500,
...
new_labels_en <- c(
  "(Intercept)" = "Intercept", 
  "age_cent" = "Age",
  "SAB_id_cent" = "Treatment experience: INPAT",
  "sexmale" = "Gender: male\n(Ref.: female)",
  ...
  "QEB_count_before_SAB_cent" = "Treatment experience: QWT",
  "REHA_count_before_SAB_cent" = "Treatment experience: REHA"
)

coefffix_plot_en <- sjPlot::plot_model(best_model, vline.color = "black", show.values = TRUE, value.offset = .4, value.size = 3.5, dot.size = 2, sort.est = TRUE, axis.labels = new_labels_en, title = "", color = c("blue", "red"),  axis.lim = c(0.6, 3), wrap.labels = 100)
...
#save as tif
ggsave(file.path(folder_plot, "coefffix_plot_SAB_en.tif"), coefffix_plot_en, width = 8, height = 5, dpi = 700, bg = "white")
