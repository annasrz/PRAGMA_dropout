...
qwt <- qwt %>%
  arrange(pragmaid, date.qwt.start) %>%
  group_by(pragmaid) %>%
  mutate(
    direkter_anschluss = (date.qwt.start == lag(date.qwt.end, default = NULL)) | (date.qwt.start == lag(date.qwt.end, default = NULL) + 1)) #das Startdatum ist gleich dem Enddatum oder es ist um einen Tag versetzt

table(qwt$direkter_anschluss, useNA = "always") #52

#check -> okay
qwt %>% arrange(pragmaid, date.qwt.start) %>% filter(direkter_anschluss | lead(direkter_anschluss)) %>% select(pragmaid, date.qwt.start, date.qwt.end)
...
qwt <- qwt %>% 
  ...
  mutate(
    date.qwt.start = case_when(
      direkter_anschluss ~ lag(date.qwt.start),
      TRUE ~ date.qwt.start
    )
  ) %>%
...
overlap_check <- qwt %>%
  ...
  summarize(
    ...
    any(date.qwt.start[i] < date.qwt.end[-i] & date.qwt.end[i] > date.qwt.start[-i])
  }))) %>%
  filter(overlap)  

# Anzeigen der Ergebnisse mit Überlappungen
print(overlap_check) # 13 Patienten haben überlappende Behandlungen
...
ggplot(qwt %>% filter(pragmaid %in% overlap_check$pragmaid), 
       aes(y = pragmaid)) +
  geom_segment(aes(x = date.qwt.start, 
                   xend = date.qwt.end, 
                   yend = pragmaid, 
...
cases_to_remove <- qwt %>% 
  ...
  mutate(group_overlap = cumsum(
      coalesce(
        as.logical(date.qwt.start > lag(date.qwt.end, n = 1, default = first(date.qwt.start)) &
                   date.qwt.start > lag(date.qwt.end, n = 2, default = first(date.qwt.start)) &
                   date.qwt.start > lag(date.qwt.end, n = 3, default = first(date.qwt.start))
        ), TRUE
      ))) %>%
...
cases_to_remove %>% arrange(pragmaid, date.qwt.start) %>% select(pragmaid, date.qwt.start, date.qwt.end, overlap, group_overlap, keep)
...
qwt <- qwt %>% mutate(unpl_drop = factor(case_when(
...
ENTL_ndays <- ggplot(qwt %>% filter (ENTL301 != "Tod"), aes(x = ENTL301, y = n.days)) + 
...
qwt_reha <- bind_rows(
  qwt, reha %>% select(all_of(colnames)))
...
qwt_reha_inpat <- bind_rows(qwt_reha, inpat %>% select(all_of(colnames)))
...
vif_model_ENTL <- glm(unpl_drop ~ sex + age + qwt_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + REHA_count_before_qwt + INPAT_count_before_qwt + treat_dur_cat, data = final_data, family = "binomial")
...
getSummary.mblogit(model3) #Modell 3 hat die besten Anpassungswerte basierend auf Devianz und AIC. "age" scheint ein signifikanter Prädiktor für die Kategorien „7–20 Tage“ und „21+ Tage“ im Vergleich zu „1-6 Tage“ zu sein.
...
getSummary.mblogit(model5) # Modell 5 bietet keinen Vorteil gegenüber Modell 4 (außer leicht niedrigerer deviance), nationality wird nicht in das modell aufgenommen
...
add_model_summary(model10, "Model 10") # keine Verbesserung durch hinzufügen von fost.degree_bin (außer in deviance)
...
model12 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + char_sum + ndistinctATClevel2 + REHA_count_before_qwt, random = ~ 1 | pragmaid, data = final_data)
getSummary.mblogit(model12)
add_model_summary(model12, "Model 12") 

model13 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + char_sum + ndistinctATClevel2 + REHA_count_before_qwt + INPAT_count_before_qwt, random = ~ 1 | pragmaid, data = final_data)
getSummary.mblogit(model13)
add_model_summary(model13, "Model 13") 

model14 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + char_sum + ndistinctATClevel2 + REHA_count_before_qwt + INPAT_count_before_qwt + age*qwt_id, random = ~ 1 | pragmaid, data = final_data)
getSummary.mblogit(model14) #signifikante Interaktion
...
graph2doc(x = coefffix_plot, file=file.path(folder_plot, "coefffix_plot_qwt.png"), width=7, height=5)
```
# Sensitivitätsanalyse mit unpl_drop

```{r sensitivity}
model1_ENTL <- glmmTMB(unpl_drop ~ sex + age + qwt_id + nationality + emp.type + char_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + INPAT_count_before_qwt + REHA_count_before_qwt + (1|pragmaid), family = "binomial", data = final_data) 
summary(model1_ENTL) 

coefffix_plot_ENTL <- sjPlot::plot_model(model1_ENTL, vline.color = "black", show.values = TRUE, value.offset = .4, value.size = 3, dot.size = 2, sort.est = TRUE)

graph2doc(x = coefffix_plot_ENTL, file=file.path(folder_plot, "coefffix_plot_qwt_ENTL.png"), width=7, height=5)

```
# model checking and diagnostics

```{r residuals}
model_simres <- simulateResiduals(model1_ENTL)
plot(model_simres)
```
...
result <- test_df %>%
  mutate(test_column = case_when(
    ...
    (setting != "outpatient" & date.diag.start >= (date.treat.start - months(6)) & date.diag.start <= date.qwt.start) ~ 1,
    TRUE ~ 0
  ))
#----------------------------------------------
