...
diagnoses_filtered <- all_diagnoses %>% 
  filter(pragmaid %in% total_employ$pragmaid) %>% #keep only rows that are in inpat data 
...
sum(total_diag_all_filtered$gkv.x != total_diag_all_filtered$gkv.y) 
total_diag_all_filtered <- total_diag_all_filtered %>% 
  select(-gkv.y) %>%
  rename(gkv = gkv.x)

#checks
nrow(distinct(total_diag_all_filtered, pragmaid, qwt_id))
nrow(distinct(total_diag_all, pragmaid, qwt_id)) - nrow(distinct(total_diag_all_filtered, pragmaid, qwt_id)) 
...
comorb_elix <- comorbidity::comorbidity(x=total_diag_all_filtered, id = "id_pragma_qwt",
                                code = "icd",map="elixhauser_icd10_quan",
                                assign0 = T,tidy.codes = T)

comorb_char <- comorbidity::comorbidity(x=total_diag_all_filtered, id = "id_pragma_qwt",
...
comorb_char_sum <- comorb_char %>%
  #new column with row sums using all columns except the first one and "alcohol"
  mutate(char_sum = rowSums(select(., -1))) %>%
...
sum(is.na(total_diag$char_sum)) #0 missings in n_diagnoses_wa
...
ggplot(comorb_sum, aes(x = elix_sum)) + geom_histogram(binwidth = 1)
summary(comorb_sum$elix_sum)
ggplot(comorb_sum, aes(x = char_sum)) + geom_histogram(binwidth = 1)
summary(comorb_sum$char_sum) 
```
...
sum(total_med_all_filtered$gkv.x != total_med_all_filtered$gkv.y) #0 -> gkv.x and 
...
unique_level2_ATCs <- unique(total_med_sum$Level2_ATC)
anzahl_distinkte_ATCs <- length(unique_level2_ATCs) #64

# welche Untergruppe ist am häufigsten vertreten?
sorted_atc_counts <- total_med_sum %>%
...
#check for outliers
boxplot(total_med$n_days) # no outliers
boxplot(total_med$qwt_id) # some patients did 21 qwts -> remove outliers?

#ckeck for multicollinearity
logit_model <- glm(treat_dur_bin ~ sex + age + qwt_id + nationality + emp.type + char_sum + fost.degree_bin + ndistinctATClevel2, data = total_med, family = binomial)
...
cor(total_med$elix_sum, total_med$char_sum) #0.75-> high correlation
ggplot(total_med, aes(x = elix_sum, y = char_sum)) + geom_point() + geom_smooth(method = "lm")
...
cor(total_med$char_sum, total_med$ndistinctATClevel2) #0.35
ggplot(total_med, aes(x = char_sum, y = ndistinctATClevel2)) + geom_point() + geom_smooth(method = "lm")
...
model3.7.1 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + gkv + elix_sum, random = ~ 1 | pragmaid, data = total_med)
...
model3.7.2 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + gkv + char_sum, random = ~ 1 | pragmaid, data = total_med)
getSummary.mblogit(model3.7.2) #Komorbidität nach Charlson verbessert Modell und hat siginifikanten Erklärungswert
model3.8 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + gkv + char_sum + fost.degree_bin, random = ~ 1 | pragmaid, data = total_med)
getSummary.mblogit(model3.8) #Modell 7.2 ist besser
model3.9 <- mblogit(formula = treat_dur_cat ~ sex + age + qwt_id + emp.type + gkv + char_sum + ndistinctATClevel2, random = ~ 1 | pragmaid, data = total_med)
