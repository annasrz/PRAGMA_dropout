...
# clean workspace
rm(list=ls())
packages <- c("data.table", "tidyverse", "ggplot2", "comorbidity", "car", "lme4", "ordinal", "export", "mclogit", "sjPlot", "see", "nnet", "ggeffects")
...
# Annahme INPAT entspricht einer Entgiftung (?), die wiederum normalerweise ziwschen 7 und 14 Tagen dauert (Problem: es gibt sehr lange INPATs, Verteilung außerdem ähnlich wie QWT)
...
# ALTERNATIVE 1: 7 Tage cutoff for "kein Abbruch" in inpat
#qwt_inpat <- qwt_inpat %>%
#  mutate(treat_dur_cat = factor(case_when(
#  #qwt: more than or 21 days in treatment -> kein Abbruch
#  n.days >= 21 & treatment == "qwt" ~ 3,
#  #qwt: more than or 7 days in treatment -> kein Abbruch
#  n.days >= 7 & treatment == "inpat" ~ 3,
#  #between 7 and 20 days (inclusive)
#  n.days >= 7 & n.days < 21 & treatment == "qwt" ~ 2,
#  n.days >= 4 & n.days < 7 & treatment == "inpat" ~ 2,
#  #between 0 and 6 days (inclusive)
#  n.days >= 0 & n.days < 7 & treatment == "qwt" ~ 1,
#  n.days >= 0 & n.days < 4 & treatment == "inpat" ~ 1,
#  TRUE ~ NA_real_), levels = c(1, 2, 3), labels = c("früher Abbruch","später Abbruch", "kein Abbruch")))
...
model15 <- mblogit(formula = treat_dur_cat ~ treatment + sex + sex*treatment + age + id_across_treat + id_across_treat*treatment + nationality + emp.type + elix_sum + elix_sum*treatment, random = ~ 1 | pragmaid, data = final_data, method = "PQL")
...
coefffix_plot <- sjPlot::plot_model(model15, vline.color = "black", show.values = TRUE, value.offset = .4, value.size = 3, dot.size = 2, sort.est = TRUE, title = "Fixed Effects: Predictors of Treatment Length (QWT and INPAT). Reference Group: 1-6 days.")

ggsave(file.path(folder_plot, "coefffix_plot.png"), coefffix_plot, width = 30, height = 20, units = "cm", dpi = 300)

graph2doc(x = coefffix_plot, file=file.path(folder_plot, "coefffix_plot.png"), width=7, height=5)
```
...
predict_and_plot <- function(variable_of_interest, model, data) {
  ...
  id_across_treat_values <- seq(from = min(data$id_across_treat,na.rm=TRUE),to=max(data$id_across_treat, na.rm = TRUE), length.out = 9)  # 9 Werte für inpat_id
  elix_sum_values <- seq(from = min(data$elix_sum,na.rm=TRUE),to=max(data$elix_sum, na.rm = TRUE), length.out = 5)  # 5 Werte für elix_sum
  ...
  } else if (variable_of_interest %in% colnames(data)) {
    new_data <- expand.grid(
      age = mean(data$age, na.rm=TRUE),
      treatment = unique(data$treatment),
      pragmaid = unique(data$pragmaid),
...
#    new_data$pragmaid <- levels(data$pragmaid)[12] # Nutze ein existierendes Level als Dummy
#  }
  assign("new_data", new_data, envir = .GlobalEnv)
  # Vorhersagen berechnen
  predicted_probs <- predict(model, newdata = new_data, type = "response")
  assign("predicted_probs", predicted_probs, envir = .GlobalEnv)
  ...
  assign("predicted_long", predicted_long, envir = .GlobalEnv)
  
  # vorhergesagte Wahrscheinlichkeiten zusammenfassen entsprechend der variables of interest
  
  predicted_long_sum <- predicted_long %>%
    group_by(treat_dur_cat, treatment, sex, .data[[variable_of_interest]]) %>%
    summarize(predicted_prob = mean(predicted_prob))
  assign("predicted_long_sum", predicted_long_sum, envir = .GlobalEnv)
  
  #geom depending on the variable of interest
  if (variable_of_interest %in% c("age", "id_across_treat", "elix_sum")) {
    geom <- geom_line()
  } else {
    geom <- geom_bar(stat = "identity", position = "dodge", aes(fill = .data[[variable_of_interest]]))
  }
  
  # Plotten der vorhergesagten Wahrscheinlichkeiten
# Plotten der vorhergesagten Wahrscheinlichkeiten
  ggplot(predicted_long_sum, aes(x = !!sym(variable_of_interest), y = predicted_prob, color = sex, fill = sex)) +
    {
      if (variable_of_interest %in% c("age", "id_across_treat", "elix_sum")) {
        geom_line()
      } else {
    ...
    facet_grid(treat_dur_cat ~ treatment, scales = "free") +
    xlab(x_label) +
...
predict_and_plot("emp.type", model15, final_data)

predicted_long_sum_sex <- predicted_long_sum %>% 
  group_by(treat_dur_cat, treatment, sex) %>%
  summarize(predicted_prob = mean(predicted_prob))

alltreats_predicted_probs_sex <- ggplot(predicted_long_sum_sex, aes(x=sex, y=predicted_prob, fill = sex)) +
...
ggsave(file.path(folder_plot, "alltreats_predicted_probs_id.png"), predict_and_plot("id_across_treat", model15, final_data), width = 20, height = 25, units = "cm")
ggsave(file.path(folder_plot, "alltreats_predicted_probs_age.png"), predict_and_plot("age", model15, final_data), width = 20, height = 25, units = "cm")
ggsave(file.path(folder_plot, "alltreats_predicted_probs_elix.png"), predict_and_plot("elix_sum", model15, final_data), width = 20, height = 25, units = "cm")
ggsave(file.path(folder_plot, "alltreats_predicted_probs_sex.png"), alltreats_predicted_probs_sex, width = 20, height = 25, units = "cm")
graph2doc(x = alltreats_predicted_probs_sex, file=file.path(folder_plot, "alltreats_predicted_probs_sex.png"), width=7, height=5)
graph2doc(x = predict_and_plot("id_across_treat", model15, final_data), file=file.path(folder_plot, "alltreats_predicted_probs_id.png"), width=7, height=5)
graph2doc(x = predict_and_plot("age", model15, final_data), file=file.path(folder_plot, "alltreats_predicted_probs_age.png"), width=7, height=5)
graph2doc(x = predict_and_plot("elix_sum", model15, final_data), file=file.path(folder_plot, "alltreats_predicted_probs_elix.png"), width=7, height=5)
graph2doc(x = predict_and_plot("emp.type", model15, final_data), file=file.path(folder_plot, "alltreats_predicted_probs_emp.png"), width=7, height=5)
```