...
# not up to date: elix_sum, ndistinctATClevel2, QWT_count_before_REHA, INPAT_count_before_REHA are not standardized, as they do have a meaningful zero point.

```{r standardize}  
#age
reha_final$age_std <- scale(reha_final$age)

#reha_id
reha_final$reha_id_std <- scale(reha_final$reha_id)

#elix_sum
reha_final$elix_sum_std <- scale(reha_final$elix_sum)

#ndistinctATClevel2
reha_final$ndistinctATClevel2_std <- scale(reha_final$ndistinctATClevel2)

#QWT_count_before_REHA
reha_final$QWT_count_before_REHA_std <- scale(reha_final$QWT_count_before_reha)

#INPAT_count_before_REHA
reha_final$INPAT_count_before_REHA_std <- scale(reha_final$INPAT_count_before_reha)

#char_sum
reha_final$char_sum_std <- scale(reha_final$char_sum)

#n.days is a bit more complicated, as n.days should be standardized by MASSN_ART (as the treatment duration is different for different treatments)
reha_final <- reha_final %>%
  group_by(MASSN_ART) %>%
  mutate(n.days_std = scale(n.days)) %>%
  ungroup()

#summary of n.days_std by MASSN_ART
for (massn in unique(reha_final$MASSN_ART)) {
  print(massn)
  print(summary(reha_final$n.days_std[reha_final$MASSN_ART == massn]))
}
...
vif_model <- glm(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + n.days_std, data = reha_final, family = binomial)

vif_model_VERLAUF <- glm(VERLAUF ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + n.days_std, data = reha_final, family = binomial)
...
modeltest1 <- glmmTMB(unpl_drop ~ n.days_std + n.days_std*MASSN_ART + (1|pragmaid), family = "binomial", data = reha_final)
summary(modeltest1)
...
model3 <- glmmTMB(unpl_drop ~ sex + age_std + (1|pragmaid), family = "binomial", data = reha_final)
summary(model3)
add_model_summary(model3, "Model 3")

model4 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + (1|pragmaid), family = "binomial", data = reha_final)
summary(model4)
add_model_summary(model4, "Model 4")

model5 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + (1|pragmaid), family = "binomial", data = reha_final)
summary(model5)
add_model_summary(model5, "Model 5")

model6 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + (1|pragmaid), family = "binomial", data = reha_final)
summary(model6) # Model 6 hat besseres AIC und deviance als Modell 3, aber schlechteres BIC
add_model_summary(model6, "Model 6")

model7.1 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + (1|pragmaid), family = "binomial", data = reha_final)
summary(model7.1)
add_model_summary(model7.1, "Model 7.1")

model7.2 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + char_sum_std + (1|pragmaid), family = "binomial", data = reha_final) #elix sum fits slightly better 
summary(model7.2)
add_model_summary(model7.2, "Model 7.2")

model8 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + (1|pragmaid), family = "binomial", data = reha_final) #elix sum fits slightly better 
summary(model8)
add_model_summary(model8, "Model 8")


model9 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + (1|pragmaid), family = "binomial", data = reha_final) #elix sum fits slightly better 
summary(model9)
add_model_summary(model9, "Model 9")

model10 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + (1|pragmaid), family = "binomial", data = reha_final) 
summary(model10)
add_model_summary(model10, "Model 10")

model11 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + (1|pragmaid), family = "binomial", data = reha_final) 
summary(model11)
add_model_summary(model11, "Model 11")

model12 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + n.days_std + (1|pragmaid), family = "binomial", data = reha_final)
summary(model12)
add_model_summary(model12, "Model 12")


model13 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + n.days_std + age_std*reha_id_std + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model13)
add_model_summary(model13, "Model 13")

model14 <- glmmTMB(unpl_drop ~ sex + age_std + reha_id_std + STAATS_ANGE + emp.type + elix_sum_std + ndistinctATClevel2_std + MASSN_ART + QWT_count_before_REHA_std + INPAT_count_before_REHA_std + n.days_std + age_std*reha_id_std + n.days_std*MASSN_ART +
                     (1|pragmaid), family = "binomial", data = reha_final) 
summary(model14)
...
plot(ggpredict(modeltest, terms = c("n.days_std")))
plot(ggpredict(model11, terms = c("age_std")))
...
plot(ae)
plot(ae[["sex"]])
plot(ae[["age_std"]]) #signifikant
plot(ae[["reha_id_std"]])
plot(ae[["STAATS_ANGE"]])
plot(ae[["emp.type"]]) #signifikant
plot(ae[["elix_sum_std"]])
plot(ae[["ndistinctATClevel2_std"]])
plot(ae[["MASSN_ART"]]) #signifikant
plot(ae[["QWT_count_before_REHA_std"]])
plot(ae[["INPAT_count_before_REHA_std"]])

```
# Model diagnostics
```{r residuals}
model_simres <- simulateResiduals(modeltest)
plot(model_simres)


plotResiduals(model_simres, form = reha_final$MASSN_ART)
plotResiduals(model_simres, form = reha_final$age_std)
plotResiduals(model_simres, form = reha_final$reha_id_std)
