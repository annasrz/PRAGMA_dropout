...
print(names(ldf))
names <- c("pragma_id_GKV", "alc_diagnoses", "all_diagnoses", "medications", "employment", "fosterage", "income", "insurance_periods", "inpat_OPS", "inpat", "qwt_OPS", "qwt", "reha", "reha_new")
names(ldf) <- names
...
head(reha)
reha_new$n.days <- as.integer(difftime(as.Date(reha_new$date.drvreha.end), as.Date(reha_new$date.drvreha.start), units = "days")) + 1
head(reha)

summary(reha$n.days)
summary(reha_new$n.days)

ggplot(reha_new, aes(x = MASSN_ART, fill = MASSN_ART)) +
  geom_bar() +
...
prop.table(table(reha_new$unpl_drop, useNA = "ifany"))

ggplot(reha_new, aes(x = MASSN_ART, fill = unpl_drop)) +
  geom_bar() +
  facet_wrap(~DIAG_VERAEND, scales = "free_y") +
  #rotate x axis labels
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
table(reha_new$reha.typ, reha$source) #gkv: ambulant & stationär, drv: ambulant & ganztägig ambulante Maßnahme & stationäre Maßnahme
...
```{r add_employment}
nrow(distinct(reha_new, pragmaid, reha_id)) #5293 Fälle
employ_unfiltered <- left_join(reha_new, employment, by = "pragmaid")
reha_new %>%
  filter(pragmaid %in% employment$pragmaid) %>%
  print()
nrow(distinct(total_stamm, pragmaid, inpat_id)) # 3887 cases
#FEHLER: pragmaIDs matchen nicht.
```
# Auswahl relevanter Variablen

```{r select_vars}
reha_final <- reha_new %>% 
  select(-c(source, DIAG_STELLEN, DIAG_ALLES, DIAG_SEITE, DIAG_GRUPPE, nationality, DAUER_REHA, DIAG1, BEWILLDIAG, date.drvreha.start, date.drvreha.end, FAMILIE, ERWERB_V_ANTR, STELLUNG_BERUF, LEISTUNGSART, yob))
```
