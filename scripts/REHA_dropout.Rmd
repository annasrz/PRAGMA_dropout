...
ldf <- lapply(names(ldf), function(df_name) {
  ...
  if (df_name %in% c("drvreha", "SAB", "QEB", "reha")) {
...
# Merge Data
# drvreha_gkvinsured_full <- bind_rows(
#   drvreha_gkvinsured,
#   reha %>%
#     filter(source == "gkv") %>%
#     select(pragmaid, date.treat.start, treatment, date.treat.end))
...
drvreha_gkvinsured <- drvreha_gkvinsured %>%
  ...
  mutate(
    reha_id = if_else(
      treatment == "drvreha",
      cumsum(lag(treatment %in% c("drvreha"), default = FALSE)),
      NA_integer_
    ),
    # Zähler: wie viele vorherige drvreha-Behandlungen wurden abgebrochen?
    n_REHA_abgebrochen = if_else(
      treatment == "drvreha",
      cumsum(lag(treatment == "drvreha" & unpl_drop == "ja", default = FALSE)),
      NA_integer_
    ),
    n_REHA_regulaer = if_else(
      treatment == "drvreha",
      cumsum(lag(treatment == "drvreha" & unpl_drop == "nein", default = FALSE)),
      NA_integer_
    ))


#check
drvreha_gkvinsured %>% select(pragmaid, date.treat.start, date.treat.end, reha_id, treatment, n_REHA_abgebrochen, unpl_drop) %>% arrange(pragmaid, date.treat.start)

table(drvreha_gkvinsured$reha_id, useNA = "always")
table(drvreha_gkvinsured$n_REHA_abgebrochen, useNA = "always")
table(drvreha_gkvinsured$n_REHA_regulaer, useNA = "always")
#table(drvreha_gkvinsured$reha_id, drvreha_gkvinsured_full$n_REHA_abgebrochen, useNA = "always")

# drvreha_gkvinsured_full <- drvreha_gkvinsured_full %>%
#   filter(treatment == "drvreha")
...
#one case that is insured but have no valid employment data. add them again to dataset and set employment to "employed" according to ERWERB_V_ANTRAG
...
# categorize ETNL301 to dropout in QEB 
labels_entl301 <- c(
  "1" = "regulär",
...
med_diagnoses_reha_employ_QEB <- bind_rows(
  med_diagnoses_reha_employ, QEB %>% select(pragmaid, date.treat.start, treatment, date.treat.end, unpl_drop))

med_diagnoses_reha_employ_QEB <- med_diagnoses_reha_employ_QEB %>%
  group_by(pragmaid) %>%
  arrange(date.treat.start) %>%
  mutate(
    QEB_count_before_reha = sapply(1:n(), function(i) {
    ...
    n_QEB_abgebrochen = sapply(1:n(), function(i) {
      if (treatment[i] == "drvreha") {
        sum(treatment[1:(i-1)] == "QEB" & unpl_drop[1:(i-1)] == "ja" & date.treat.end[1:(i-1)] < date.treat.start[i])
      } else {
        NA
      }
    }),
    n_QEB_regulaer = sapply(1:n(), function(i) {
      if (treatment[i] == "drvreha") {
        sum(treatment[1:(i-1)] == "QEB" & unpl_drop[1:(i-1)] == "nein" & date.treat.end[1:(i-1)] < date.treat.start[i])
      } else {
        NA
      }
    })) %>%
  ...
  #   unpl_within_12m_before = sapply(1:n(), function(i) {
  #     if (treatment[i] == "drvreha") {
  #       any(
  #         treatment[1:(i-1)] %in% c("QEB", "drvreha") &
  #         unpl_drop[1:(i-1)] == "ja" &
  ...
  #     }
  #   }),
  #   unpl_within_12m_before = factor(
  #     ifelse(is.na(unpl_within_12m_before), NA_integer_, as.integer(unpl_within_12m_before)),
  #     levels = c(0, 1),
  #     labels = c("nein", "ja")
  #   )
  # )
...
#check
med_diagnoses_reha_employ_QEB %>% select(pragmaid, date.treat.start, reha_id, treatment, QEB_count_before_reha, n_QEB_abgebrochen, n_REHA_abgebrochen, unpl_drop) %>% arrange(pragmaid, date.treat.start)
table(med_diagnoses_reha_employ_QEB[med_diagnoses_reha_employ_QEB$treatment == "drvreha",]$QEB_count_before_reha, useNA = "always")
table(med_diagnoses_reha_employ_QEB[med_diagnoses_reha_employ_QEB$treatment == "drvreha",]$n_QEB_abgebrochen, useNA = "always")
table(med_diagnoses_reha_employ_QEB[med_diagnoses_reha_employ_QEB$treatment == "drvreha",]$n_QEB_regulaer, useNA = "always")
...
med_diagnoses_reha_employ_QEB_SAB <- bind_rows(
  med_diagnoses_reha_employ_QEB, SAB %>% select(pragmaid, date.treat.start, treatment, date.treat.end, unpl_drop)) 

med_diagnoses_reha_employ_QEB_SAB <- bind_rows(
  med_diagnoses_reha_employ_QEB,
  SAB %>% select(pragmaid, date.treat.start, treatment, date.treat.end, unpl_drop)) %>%
  arrange(pragmaid, date.treat.start) %>%
  group_by(pragmaid) %>%
    mutate(
    SAB_count_before_reha = sapply(1:n(), function(i) {
      if (treatment[i] == "drvreha") {
    ...
    n_SAB_regulaer = sapply(1:n(), function(i) {
      if (treatment[i] == "drvreha") {
        sum(treatment[1:(i-1)] == "SAB" & unpl_drop[1:(i-1)] == "nein" & date.treat.end[1:(i-1)] < date.treat.start[i])
      } else {
        NA
      }
    })
...
#med_diagnoses_reha_employ_QEB_SAB <- med_diagnoses_reha_employ_QEB_SAB %>% ungroup()
  # 
  # 
  # 
  # 
  ...
  # ) %>%
  # 
  # # Hilfsvariable: SAB-Abbrüche innerhalb der letzten 12 Monate 
  # mutate(
  ...
  #   }),
  #   
  #   # in den kletzten 12 Monaten kein Abbruch ans den vorherigen Treatments (="nein") und jetzt SAB-Abbruch: auf "ja" setzen
  #   unpl_within_12m_before = case_when(
  #     sab_unpl_before & unpl_within_12m_before == "nein" ~ "ja",
  #     TRUE ~ unpl_within_12m_before
  #   ),
  #   
  #   unpl_within_12m_before = factor(unpl_within_12m_before, levels = c("nein", "ja"))
  # ) %>%
  # 
  # ungroup()
...
table(med_diagnoses_reha_employ_QEB_SAB[med_diagnoses_reha_employ_QEB_SAB$treatment == "drvreha",]$SAB_count_before_reha, useNA = "always")
table(med_diagnoses_reha_employ_QEB_SAB[med_diagnoses_reha_employ_QEB_SAB$treatment == "drvreha",]$n_SAB_abgebrochen, useNA = "always")
# med_diagnoses_reha_employ_QEB_SAB %>% select(pragmaid, date.treat.start, reha_id, treatment, QEB_count_before_reha, SAB_count_before_reha, sab_unpl_before, unpl_within_12m_before, unpl_drop) %>% arrange(pragmaid, date.treat.start)
# table(med_diagnoses_reha_employ_QEB_SAB$unpl_within_12m_before)
# table(med_diagnoses_reha_employ_QEB_SAB$sab_unpl_before)
...
variables_to_center <- c("age", "elix_sum", "ndistinctATClevel2", "n_SAB_abgebrochen", "n_QEB_abgebrochen", "n_REHA_abgebrochen", "n_SAB_regulaer", "n_QEB_regulaer", "n_REHA_regulaer", "n_treats") 
...
```{r correlation}
# correlation matrix
cent_vars <- names(reha_final)[grepl("_cent$", names(reha_final))]

cor_matrix <- cor(reha_final %>% select(all_of(cent_vars)), use = "pairwise.complete.obs")
...
reha_final$logn_SAB_abgebrochenInt <- log(reha_final$n_SAB_abgebrochen+1) * reha_final$n_SAB_abgebrochen
reha_final$logn_QEB_abgebrochenInt <- log(reha_final$n_QEB_abgebrochen+1) * reha_final$n_QEB_abgebrochen
reha_final$logn_REHA_abgebrochenInt <- log(reha_final$n_REHA_abgebrochen+1) * reha_final$n_REHA_abgebrochen
reha_final$logn_SAB_regulaerInt <- log(reha_final$n_SAB_regulaer+1) * reha_final$n_SAB_regulaer
reha_final$logn_QEB_regulaerInt <- log(reha_final$n_QEB_regulaer+1) * reha_final$n_QEB_regulaer
reha_final$logn_REHA_regulaerInt <- log(reha_final$n_REHA_regulaer+1) * reha_final$n_REHA_regulaer


outcomes <- c("unpl_drop")


predictors <- "sex + STAATS_ANGE + MASSN_ART + emp.type + age + logageInt + elix_sum + logelix_sumInt + ndistinctATClevel2 + logndistinctATClevel2Int + n_SAB_abgebrochen + logn_SAB_abgebrochenInt + n_QEB_abgebrochen + logn_QEB_abgebrochenInt + n_REHA_abgebrochen + logn_REHA_abgebrochenInt + n_SAB_regulaer + logn_SAB_regulaerInt + n_QEB_regulaer + logn_QEB_regulaerInt + n_REHA_regulaer + logn_REHA_regulaerInt"
...
Es gibt keine signifikanten Interaktionseffekte, die darauf hindeuten, dass die Annahme der Linearität im Logit verletzt ist. Da die counter für Prädiktoren Anzahl vorheriger Behandlungen und Anzahl vorheriger abgerochener Variablen zero-inflated und schief sind, werden diese kategorisiert.

### Kategorisierung von Prädiktoren
```{r categorization}
# 
# #Anzahl vorangeganer SABs
# table(reha_final$SAB_count_before_reha, useNA = "always")
# 
# limits_1 <- quantile(reha_final$SAB_count_before_reha[reha_final$SAB_count_before_reha > 0], probs = c(0.5))
# 
# reha_final <- reha_final %>% mutate(
#   SAB_count_before_reha_cat = case_when(
#     SAB_count_before_reha == 0 ~ "0",
#     SAB_count_before_reha <= tertile_limits_1[1] ~ "1",
#     TRUE ~ "2+") %>% factor(levels = c("0", "1", "2+"))
# )
# 
# # Check
# table(reha_final$SAB_count_before_reha_cat, reha_final$SAB_count_before_reha, useNA = "always")
# 
# #Anzahl abgebrochener SABs
# table(reha_final$n_SAB_abgebrochen, useNA = "always")
# 
# limits_2 <- quantile(reha_final$n_SAB_abgebrochen[reha_final$n_SAB_abgebrochen > 0], probs = c(0.5))
# 
# reha_final <- reha_final %>% mutate(
#   n_SAB_abgebrochen_cat = case_when(
#     n_SAB_abgebrochen == 0 ~ "0",
#     n_SAB_abgebrochen <= limits_2[1] ~ "1",
#     TRUE ~ "2+"
#   ) %>% factor(levels = c("0", "1", "2+"))
# )
# 
# #Anzahl vorangegangener rehas
# table(reha_final$reha_id, useNA = "always")
# limits_3 <- quantile(reha_final$reha_id[reha_final$reha_id > 0], probs = c(0.5))
# reha_final <- reha_final %>% mutate(
#   reha_id_cat = case_when(
#     reha_id == 0 ~ "0",
#     reha_id <= limits_3[1] ~ "1",
#     TRUE ~ "2+"
#   ) %>% factor(levels = c("0", "1", "2+"))
# )
# 
# #Anzahl abgebrochener rehas
# table(reha_final$n_REHA_abgebrochen, useNA = "always")
# reha_final <- reha_final %>% mutate(
#   n_REHA_abgebrochen_cat = case_when(
#     n_REHA_abgebrochen == 0 ~ "0",
#     TRUE ~ "1+"
#   ) %>% factor(levels = c("0", "1+"))
# )
# 
# #Anzahl vorangegangener QEBs
# table(reha_final$QEB_count_before_reha, useNA = "always")
# limits_4 <- quantile(reha_final$QEB_count_before_reha[reha_final$QEB_count_before_reha > 0], probs = c(0.5))
# 
# reha_final <- reha_final %>% mutate(
#   QEB_count_before_reha_cat = case_when(
#     QEB_count_before_reha == 0 ~ "0",
#     QEB_count_before_reha <= limits_4[1] ~ "1",
#     TRUE ~ "2+"
#   ) %>% factor(levels = c("0", "1", "2+"))
# )
# 
# #Anzahl abgebrochener QEBs
# table(reha_final$n_QEB_abgebrochen, useNA = "always")
# reha_final <- reha_final %>% mutate(
...
# 
# # Check
# table(reha_final$n_QEB_abgebrochen_cat, useNA = "always")
```
...
vif_model <- glm(unpl_drop ~ sex + age_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + n_REHA_abgebrochen_cent+ n_QEB_abgebrochen_cent + n_SAB_abgebrochen_cent
                 + n_REHA_regulaer_cent + n_QEB_regulaer_cent + n_SAB_regulaer_cent, data = reha_final, family = binomial)
car::vif(vif_model)
...
model4 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + (1|pragmaid), family = "binomial", data = reha_final)
summary(model4)
add_model_summary(model4, "Model 4")

model5 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + (1|pragmaid), family = "binomial", data = reha_final)
summary(model5)
add_model_summary(model5, "Model 5")

model6 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + (1|pragmaid), family = "binomial", data = reha_final)
summary(model6) 
add_model_summary(model6, "Model 6")

model7 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + (1|pragmaid), family = "binomial", data = reha_final)
summary(model7)
add_model_summary(model7, "Model 7")

model8 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + (1|pragmaid), family = "binomial", data = reha_final)
summary(model8)
add_model_summary(model8, "Model 8")

model9 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + (1|pragmaid), family = "binomial", data = reha_final) 
summary(model9)
add_model_summary(model9, "Model 9")

model10 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + (1|pragmaid), family = "binomial", data = reha_final) 
summary(model10)
add_model_summary(model10, "Model 10")

model11 <- glmmTMB(unpl_drop ~ sex + age_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + n_REHA_abgebrochen_cent+ n_SAB_abgebrochen_cent +n_QEB_abgebrochen_cent + 
                 + n_REHA_regulaer_cent + n_QEB_regulaer_cent + n_SAB_regulaer_cent + (1|pragmaid), family = "binomial", data = reha_final) 
summary(model11)
add_model_summary(model11, "Model 11")
#plot(ggeffects::ggpredict(model11, terms = c("SAB_count_before_reha_cent", "n_SAB_abgebrochen_cent")))

# model12 <- glmmTMB(unpl_drop ~ sex + age_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART*emp.type + n_REHA_abgebrochen_cent+ n_SAB_abgebrochen_cent +n_QEB_abgebrochen_cent + 
#                  + n_REHA_regulaer_cent + n_QEB_regulaer_cent + n_SAB_regulaer_cent + (1|pragmaid), family = "binomial", data = reha_final) 
# summary(model12)
...
predictors <- c("sex", "age_cent", "STAATS_ANGE", "emp.type", "elix_sum_cent", 
                "ndistinctATClevel2_cent", "MASSN_ART")
...
for (pred in predictors) {
  ...
  # Modell berechnen
  model <- glmmTMB(f, family = "binomial", data = reha_final)
...
model13 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent + age_cent*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model13)
add_model_summary(model13, "Model 13") 

model14 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent +  sex*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model14)
add_model_summary(model14, "Model 14")

model15 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent +  STAATS_ANGE*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model15)
add_model_summary(model15, "Model 15")

model16 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent +  emp.type*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model16) 
add_model_summary(model16, "Model 16")

# model17 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent + SAB_count_before_reha_cent +unpl_within_12m_before+ emp.type*reha_id_cent + (1|pragmaid), family = 
#                      "binomial", data = reha_final) 
# summary(model17)
# add_model_summary(model17, "Model 17")

model18 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent +  elix_sum_cent*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model18) 
add_model_summary(model18, "Model 18")

model19 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent +  ndistinctATClevel2_cent*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final) 
summary(model19) 
add_model_summary(model19, "Model 19")

model20 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent + MASSN_ART*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final)
summary(model20)
add_model_summary(model20, "Model 20")

model21 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent + QEB_count_before_reha_cent*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final)
summary(model21) 
add_model_summary(model21, "Model 21")

model22 <- glmmTMB(unpl_drop ~ sex + age_cent + reha_id_cent*n_REHA_abgebrochen_cent + STAATS_ANGE + emp.type + elix_sum_cent + ndistinctATClevel2_cent + MASSN_ART + QEB_count_before_reha_cent*n_QEB_abgebrochen_cent + SAB_count_before_reha_cent*n_SAB_abgebrochen_cent + SAB_count_before_reha_cent*reha_id_cent + (1|pragmaid), family = 
                     "binomial", data = reha_final)
summary(model22) 
...
#unpl_within_12m_before
#prop.table(table(reha_final$unpl_within_12m_before))
...
#anzahl distinkter pragmaIDs mit reha_id > 0 / anzahl aller distinkter pragmaIDs = anteil der patienten mit mehr als einer reha
(reha_final %>% filter(n_REHA_abgebrochen > 0 | n_REHA_regulaer) %>% distinct(pragmaid) %>% nrow()) / n_pat 
#mehr als 2 reha
...
#REHA
n_treats_reha <- reha_final %>% group_by(pragmaid, unpl_drop) %>% summarise(n_treats = n()) %>% pull(n_treats)
...
new_labels <- c(
  "(Intercept)" = "Intercept", 
  "age_cent" = "Alter",
  "n_REHA_abgebrochen_cent" = "vorherige irregulär beendete REHA (Anzahl, kont.)",
  "n_QEB_abgebrochen_cent" = "vorherige irregulär beendete QEB (Anzahl, kont.)",
  "n_SAB_abgebrochen_cent" = "vorherige irregulär beendete SAB (Anzahl, kont.)",
  "n_REHA_regulaer_cent" = "vorherige regulär beendete REHA (Anzahl, kont.)",
  "n_QEB_regulaer_cent" = "vorherige regulär beendete QEB (Anzahl, kont.)",
  "n_SAB_regulaer_cent" = "vorherige regulär beendete SAB (Anzahl, kont.)",
#  "reha_id_cent" = "Behandlungserfahrung REHA",
  "sexmale" = "Geschlecht: männlich (Ref.: weiblich)",
  "STAATS_ANGEnicht deutsch" = "Nationalität: deutsch (Ref.: nicht-deutsch)",
  "emp.typeother" = "Beschäftigungsstatus: andere (Ref.: erwerbstätig)",
  "emp.typeunemployed" = "Beschäftigungsstatus: arbeitslos (Ref.: erwerbstätig)",
...
#  "unpl_within_12m_beforeja" = "Mind. 1 vorherige irreg. Beendigung\n(Ref.: keine)",
  "MASSN_ARTstationäre Maßnahme" = "Behandlungsart: stationär (Ref.: ambulant)",
  "MASSN_ARTganztägig ambulante Maßnahme" = "Behandlungsart: ganztägig ambulant (Ref.: ambulant)"
)
...
#save as tif
ggsave(file.path(folder_plot, "coefffix_plot_REHA.tif"), coefffix_plot, 
       width = 175,
       height = 142, 
       units = "mm",
       dpi = 500,
