...
names <- c("pragma_id_GKV", "all_diagnoses", "medications", "employment", "fosterage", "income", "insurance_periods", "SAB_OPS", "SAB", "qwt_OPS", "qwt", "reha", "reha_new")
...
ldf <- lapply(names(ldf), function(df_name) {
  ...
  if (df_name %in% c("reha_new", "reha", "SAB", "qwt")) {
  ...
  # mutate values in treatment column (inpat -> SAB)
  if ("treatment" %in% colnames(df)) {
    df <- df %>%
      mutate(treatment = ifelse(as.character(treatment) == "inpat", "SAB", as.character(treatment))) %>%
...
diagnoses_SAB_employ_fil <- diagnoses_SAB_employ_alltimes %>%
  filter(
    # outpatient diagnoses (duration: 1 quarter) must not have been made more than 2 quarters before the start of QWT and must not be later than the start of QWT
...
med_fost_diagnoses_SAB_employ_qwt <- bind_rows(
  med_fost_diagnoses_SAB_employ, qwt %>% select(pragmaid, date.treat.start, treatment, date.treat.end))

med_fost_diagnoses_SAB_employ_qwt <- med_fost_diagnoses_SAB_employ_qwt %>%
  group_by(pragmaid) %>%
  arrange(date.treat.start) %>%
  mutate(QWT_count_before_SAB = sapply(1:n(), function(i) {
    if (treatment[i] == "SAB") {
...
#check
med_fost_diagnoses_SAB_employ_qwt %>% select(pragmaid, date.treat.start, SAB_id, treatment, QWT_count_before_SAB) %>% arrange(pragmaid, date.treat.start) 

med_fost_diagnoses_SAB_employ_qwt <- med_fost_diagnoses_SAB_employ_qwt %>% filter(treatment == "SAB")
...
med_fost_diagnoses_SAB_employ_qwt_reha <- bind_rows(
  med_fost_diagnoses_SAB_employ_qwt, reha %>% select(pragmaid, date.treat.start, treatment, date.treat.end))

med_fost_diagnoses_SAB_employ_qwt_reha <- med_fost_diagnoses_SAB_employ_qwt_reha %>%
  group_by(pragmaid) %>%
  arrange(date.treat.start) %>%
...
#check
med_fost_diagnoses_SAB_employ_qwt_reha %>% select(pragmaid, date.treat.start, SAB_id, treatment, QWT_count_before_SAB, REHA_count_before_SAB) %>% arrange(pragmaid, date.treat.start) 

med_fost_diagnoses_SAB_employ_qwt_reha <- med_fost_diagnoses_SAB_employ_qwt_reha %>% filter(treatment == "SAB")
```

## Auswahl relevanter Variablen

```{r select_vars}
final_data <- med_fost_diagnoses_SAB_employ_qwt_reha %>% 
...
variables_to_center <- c("age", "SAB_id", "elix_sum", "ndistinctATClevel2", "QWT_count_before_SAB", "REHA_count_before_SAB", "n.days")
...
```{r correlation}
cor_matrix <-  cor(final_data %>% select(c("age_cent", "SAB_id_cent", "elix_sum_cent", "ndistinctATClevel2_cent", "QWT_count_before_SAB_cent", "REHA_count_before_SAB_cent", "n.days_cent")))
cor_matrix
...
#QWT_count_before_SAB
boxplot(final_data$QWT_count_before_SAB)
...
final_data$logQWT_count_before_SABInt <- log(final_data$QWT_count_before_SAB+1) * final_data$QWT_count_before_SAB
...
predictors <- "sex + nationality + emp.type + age + fost.degree_bin + logageInt + SAB_id + logSAB_idInt + elix_sum + logelix_sumInt + ndistinctATClevel2 + logndistinctATClevel2Int + QWT_count_before_SAB + logQWT_count_before_SABInt + REHA_count_before_SAB + logREHA_count_before_SABInt"
...
```{r multicollinearity}
vif_model <- glm(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent, family = "binomial", data = final_data)
...
model12 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model12)
add_model_summary(model12, "Model 12") 

model13 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data) 
summary(model13)
...
```{r interactions}
model14 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent + nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*age_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model14)
add_model_summary(model14, "Model 14")

model15 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*sex + (1|pragmaid), family = "binomial", data = final_data) 
summary(model15)
add_model_summary(model15, "Model 15") 

model16 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*elix_sum_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model16)
add_model_summary(model16, "Model 16") 

model17 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*nationality + (1|pragmaid), family = "binomial", data = final_data) 
summary(model17) 
add_model_summary(model17, "Model 17") 

model_18 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*emp.type + (1|pragmaid), family = "binomial", data = final_data)
summary(model_18) 
add_model_summary(model_18, "Model 18")

model_19 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*fost.degree_bin + (1|pragmaid), family = "binomial", data = final_data)
summary(model_19)
add_model_summary(model_19, "Model 19")

model_20 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*ndistinctATClevel2_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model_20)
add_model_summary(model_20, "Model 20")

model_21 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*QWT_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model_21) 
add_model_summary(model_21, "Model 21")

model_22 <- glmmTMB(unpl_drop ~ sex + age_cent + SAB_id_cent+ nationality + emp.type + elix_sum_cent + fost.degree_bin + ndistinctATClevel2_cent + QWT_count_before_SAB_cent + REHA_count_before_SAB_cent + SAB_id_cent*REHA_count_before_SAB_cent + (1|pragmaid), family = "binomial", data = final_data)
summary(model_22) 
...
#QWT_count_before_SAB
summary(final_data$QWT_count_before_SAB)
...
n_treats_qwt <- final_data %>%
  group_by(pragmaid) %>%
  slice_max(QWT_count_before_SAB, with_ties = FALSE) %>% 
  ungroup() %>%
  pull(QWT_count_before_SAB)
...
new_labels <- c(
  ...
  "QWT_count_before_SAB_cent" = "Behandlungserfahrung QWT",
...
#save plot
graph2doc(x = coefffix_plot, file=file.path(folder_plot, "coefffix_plot_SAB.png"), width=7.5, height=4.5)
```
