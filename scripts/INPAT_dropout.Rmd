...
final_data <- med_fost_diagnoses_inpat_employ %>% 
  select(-c(KH_FALL_ID, date.inpat.start, date.inpat.end, date.emp.start, date.emp.end, id_pragma_inpat, fost.degree, source, died, date.death, yob))
```
...
vif_model <- glm(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat, family = "binomial", data = final_data)
...
model12 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + inpat_id*age + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion inpat_id*age verbessert AIC und LogLik leicht, Interaktion ist aber knapp nicht signifikant
summary(model12)
add_model_summary(model12, "Model 12") 

model13 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + inpat_id*age + inpat_id*sex + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion inpat_id*age verbessert AIC und LogLik leicht, Interaktion ist aber knapp nicht signifikant
summary(model13)
add_model_summary(model13, "Model 13") 

d12 <- as.numeric(-2*logLik(model12)) #is the -2LL for mod0
d13 <- as.numeric(-2*logLik(model13)) #is the -2LL for mod2
ldif <- d12 - d13 #is the -2LL difference
degfr <- 1 #difference in the number of parameters. degfr is 1 because mod13 has one extra parameter
##compute the p-value.
pchisq(ldif, degfr, lower.tail = FALSE) #.075
#the -2LL of Model13 is not significantly closer to 0

Anova(model12)
Anova(model12,type="III") # to do: set sum-to-zero contrasts on factors and center numerical covariates

model14 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + inpat_id*age + inpat_id*sex + inpat_id*elix_sum + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion inpat_id*elix*sum verbessert modelfit nicht
summary(model14)
add_model_summary(model14, "Model 14")


model15 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + inpat_id*age + inpat_id*sex + sex*age + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion sex*age verbessert modelfit nicht
summary(model15)
...
coefffix_plot <- sjPlot::plot_model(model12, vline.color = "black", show.values = TRUE, value.offset = .4, value.size = 3, dot.size = 2, sort.est = TRUE)

plot(ggpredict(model12, terms = c("age", "inpat_id")))


#effekte der prädiktoren unter konstanthaltung aller anderen prädiktoren
ae <- allEffects(model12)
plot(ae)
