...
inpat <- inpat %>%
  arrange(pragmaid, date.inpat.start) %>%
  group_by(pragmaid) %>%
  mutate(
    direkter_anschluss = (date.inpat.start == lag(date.inpat.end, default = NULL)) | (date.inpat.start == lag(date.inpat.end, default = NULL) + 1)) #das Startdatum ist gleich dem Enddatum oder es ist um einen Tag versetzt

table(inpat$direkter_anschluss, useNA = "always") #14 with direkter anschluss

#check -> okay
inpat %>% arrange(pragmaid, date.inpat.start) %>% filter(direkter_anschluss | lead(direkter_anschluss)) %>% select(pragmaid, date.inpat.start, date.inpat.end)

inpat <- inpat %>% 
  group_by(pragmaid) %>%
  arrange(date.inpat.start) %>%
  mutate(
    date.inpat.start = case_when(
      direkter_anschluss ~ lag(date.inpat.start),
      TRUE ~ date.inpat.start
    )
  ) %>%
...
overlap_check <- inpat %>%
  ...
  summarize(
    ...
    any(date.inpat.start[i] < date.inpat.end[-i] & date.inpat.end[i] > date.inpat.start[-i])
  }))) %>%
  filter(overlap)  
...
ggplot(inpat %>% filter(pragmaid %in% overlap_check$pragmaid), 
       aes(y = pragmaid)) +
  geom_segment(aes(x = date.inpat.start, 
                   xend = date.inpat.end, 
                   yend = pragmaid, 
...
cases_to_remove <- inpat %>% 
  ...
  mutate(group_overlap = cumsum(
      coalesce(
        as.logical(date.inpat.start > lag(date.inpat.end, n = 1, default = first(date.inpat.start)) &
                   date.inpat.start > lag(date.inpat.end, n = 2, default = first(date.inpat.start)) &
                   date.inpat.start > lag(date.inpat.end, n = 3, default = first(date.inpat.start))
        ), TRUE
      ))) %>%
...
cases_to_remove %>% arrange(pragmaid, date.inpat.start) %>% select(pragmaid, date.inpat.start, date.inpat.end, overlap, group_overlap, keep)
...
#Einteilung der ENTLASS Gründe in "regulär" vs "nicht regulär" (wenig sensitiv, aber spezifisch, dh wahrscheinlich werden viele Fälle verpasst, die ungeplant abgebrochen haben, aber dafür wenig falsch positive, dh hohe spezifität)
inpat <- inpat %>% 
  filter(ENTL301 != "Tod") %>%
...
med_fost_diagnoses_inpat_employ$treatment <- "inpat"
qwt$treatment <- "qwt"

#rename 
qwt <- qwt %>% rename(date.treat.start = date.qwt.start, date.treat.end = date.qwt.end)
med_fost_diagnoses_inpat_employ <- med_fost_diagnoses_inpat_employ %>% rename(date.treat.start = date.inpat.start, date.treat.end = date.inpat.end)
...
med_fost_diagnoses_inpat_employ_qwt <- bind_rows(
  med_fost_diagnoses_inpat_employ, qwt %>% select(all_of(colnames)))
...
#rename 
reha <- reha %>% rename(date.treat.start = date.reha.start, date.treat.end = date.reha.end)

#are colnames in qwt and inpat? 
all(colnames %in% colnames(reha))

med_fost_diagnoses_inpat_employ_qwt_reha <- bind_rows(
  med_fost_diagnoses_inpat_employ_qwt, reha %>% select(all_of(colnames)))
...
vif_model <- glm(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT, family = "binomial", data = final_data)
...
model3 <- glmmTMB(unpl_drop ~ sex + age + (1|pragmaid), family = "binomial", data = final_data)
summary(model3)
add_model_summary(model3, "Model 3")

model4 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + (1|pragmaid), family = "binomial", data = final_data)
summary(model4) # Modell 4 bietet keinen Vorteil gegenüber Modell 3 -> INPAT ID rauslassen?
add_model_summary(model4, "Model 4")

model5 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + (1|pragmaid), family = "binomial", data = final_data)
...
model6 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + (1|pragmaid), family = "binomial", data = final_data)
summary(model6) # Model 6 hat besseres AIC und deviance als Modell 3, aber schlechteres BIC
add_model_summary(model6, "Model 6")

model7 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + (1|pragmaid), family = "binomial", data = final_data)
summary(model7)
add_model_summary(model7, "Model 7")


# ist char_sum ein besserer Prädiktor?
model8 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + char_sum + (1|pragmaid), family = "binomial", data = final_data)
summary(model8)
add_model_summary(model8, "Model 8") # char_sum passt schlechter als elix_sum, ist aber auch signifikant

model9 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + (1|pragmaid), family = "binomial", data = final_data)
summary(model9)
add_model_summary(model9, "Model 9") # keine Verbesserung gegenüber Modell 8

model10 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + (1|pragmaid), family = "binomial", data = final_data)
summary(model10)
add_model_summary(model10, "Model 10") 

model11 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + (1|pragmaid), family = "binomial", data = final_data)
summary(model11)
add_model_summary(model11, "Model 11") 


model12 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + (1|pragmaid), family = "binomial", data = final_data)
summary(model12)
add_model_summary(model12, "Model 12") 

model13 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von REHA_count_before_INPAT ist nicht signifikant und verschlechtert model fit (auch wenn Reihenfolge umgekehr, dh wenn erst REHA count und dann QWT count aufgenommen wird)
summary(model13)
add_model_summary(model13, "Model 13") 

model14 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT + inpat_id*age + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion inpat_id*age verbessert AIC und LogLik leicht, Interaktion ist signifikant
summary(model14)
add_model_summary(model14, "Model 14") 

model15 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT + inpat_id*age + inpat_id*sex + (1|pragmaid), family = "binomial", data = final_data)

summary(model15)
add_model_summary(model15, "Model 15") 
#Ist Modelverbesserung signifikant durch Hinzunahme von inpat_id*sex?
...
model16 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT + inpat_id*age + inpat_id*elix_sum + (1|pragmaid), family = "binomial", data = final_data) summary(model16) #Hinzunahme von Interaktion inpat_id*elix*sum verbessert modelfit nicht
add_model_summary(model16, "Model 16") 


model17 <- glmmTMB(unpl_drop ~ sex + age + inpat_id + nationality + emp.type + elix_sum + fost.degree_bin + ndistinctATClevel2 + treat_dur_cat + QWT_count_before_INPAT + REHA_count_before_INPAT + inpat_id*age + sex*age + (1|pragmaid), family = "binomial", data = final_data) #Hinzunahme von Interaktion inpat_id*age verbessert AIC und LogLik leicht, Interaktion ist signifikant
summary(model17) #Hinzunahme von Interaktion sex*age verbessert modelfit nicht
add_model_summary(model17, "Model 17") 


#Interpretation: Bei einer geringen Anzahl von Behandlungen (niedrige inpat_id) nimmt die Wahrscheinlichkeit eines Abbruchs stärker mit dem Alter ab. Bei einer höheren Anzahl von Behandlungen ist der Einfluss des Alters weniger stark.
...
choose_best_model(model_summaries, "LogLik") #Model 11 hat den besten fit
...
coefffix_plot <- sjPlot::plot_model(model13, vline.color = "black", show.values = TRUE, value.offset = .4, value.size = 3, dot.size = 2, sort.est = TRUE)

plot(ggpredict(model14, terms = c("age", "inpat_id")))
...
plot(ae["age:inpat_id"])
plot(ae["sex"])
plot(ae["elix_sum"])
plot(ae["treat_dur_cat"])
plot(ae["REHA_count_before_INPAT"])
```
...
# Gemeinsamkeiten zu QWT Modellen:

-   Effekt Alter(positiv)
-   Effekt von Komorbidität (positiv) - jedoch char bei QWT vs. elix bei INPAT
-   Effekt von treatment_ID (negativ)
-   Effekt von Beschäftigung (positiv) - beschäftigt = längere Aufenthalte
...
-   signifikant negativer Koeffizient für inpat_ID -\> mit fortlaufenderID sinkt die Wahrscheinlichkeit für Behandlungen 21+ Tagen (7-20 Tage n.s.)
...
model_simres <- simulateResiduals(model12)
plot(model_simres)
```
